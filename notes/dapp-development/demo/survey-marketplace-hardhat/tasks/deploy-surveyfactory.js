...
task("deploy-surveyfactory", "deploys the survey factory contract")
  ...
  .setAction(async ({ signeraddress, surveycreationfees, verify }, hre) => {
    ...
    const fees = hre.ethers.utils.parseUnits(
      surveycreationfees.toString(),
      (unit = "ether")
    );
    ...
    console.log(
      "Survey creation fees in ETH : ",
      hre.ethers.utils.formatEther(initSurveyCreationFees.toString())
    );
